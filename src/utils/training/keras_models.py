...
def AcousticPhaseNet(input=None):
    """ Create the AcousticPhaseNet model.
    :param input: Length of each input sample as a number of points, None if unknown.
    :return: The AcousticPhaseNet model.
    """

    inputs = layers.Input(shape=(input,))

    x = layers.Reshape((input, 1))(inputs)
...
def resnet():
    ...
    # freeze all layers but the last convolution
    for layer in base.layers[:-5]:
        layer.trainable = False

    base = base(x)

    # put the data to 1D and apply an MLP on it
